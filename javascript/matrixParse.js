...
function buildTheThing(max_x, max_y, matrix) {
  var hash = {}
  matrix.map(function(subArray, i){
    var key = hash[subArray[1]]
    var value = { position: subArray[0] + 1, char: String.fromCharCode(subArray[2]) }
    if(key == undefined) { hash[subArray[1]] = [value] }
    if(key != undefined) { hash[subArray[1]].push(value) }
  });
  console.log(hash)
  Object.keys(hash).map(function(n){
    var row = hash[n]
    var string_row = ' '.repeat(max_x + 1)
    row.forEach(function(value){
      var position = value.position == 0 ? 1 : value.position
      string_row = string_row.substr(0, value.position - 1) + value.char + string_row.substr(position, string_row.length)
    });
    console.log(string_row)
    // console.log(string_row.length)
  })
}

buildTheThing(max_x, max_y, points)